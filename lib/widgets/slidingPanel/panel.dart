...
class SlidingPanel extends StatefulWidget {
  ...
  /// If non-null and true, the SlidingPanel exhibits a
  ...
  /// Allows toggling of the draggability of the SlidingPanel.
            ...
            : _gestureHandler(
                child: AnimatedBuilder(
                  ...
                  builder: (context, child) {
                    return Container(
                      width:
                          _ac.value * (widget.maxWidth - widget.minWidth) +
                              widget.minWidth,
                  ...
                  child: Stack(
                    children: <Widget>[
                      //open panel
                      Positioned(
                          left: widget.slideDirection == SlideDirection.LEFT
                              ? 0.0
                              : null,
                          right: widget.slideDirection == SlideDirection.RIGHT
                              ? 0.0
                              : null,
                            //TODO: check
                          width: MediaQuery.of(context).size.width -
  ...
  Widget _gestureHandler({required Widget child}) {
    ...
    if (widget.panel != null) {
      return GestureDetector(
        ...
        onHorizontalDragEnd: (DragEndDetails dets) =>
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (v.pixelsPerSecond.dx.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        if (v.pixelsPerSecond.dx.abs() >= kSnap * minFlingVelocity ||
            minDistance == d2Snap)
...
class PanelController {
  ...
  /// of the SlidingPanel (this property must return true before any other
  ...
  /// Requires that the SlidingPanel snapPoint property is not null
  /// (optional) duration specifies the time for the animation to complete
  /// (optional) curve specifies the easing behavior of the animation.
  Future<void> animatePanelToSnapPoint(
    ...
    assert(_panelState!.widget.snapPoint != null,
        "SlidingPanel snapPoint property must not be null");
    return _panelState!
